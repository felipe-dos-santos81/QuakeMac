...
is behind `#ifdef QUAKE_MCP`, so the plain `make build-release` binary is
unchanged. QuakeWorld is untouched.
...
```
quake_start(profile="local", port=28900)                 # -> instance
quake_game(instance, operation="new_game")
quake_control(instance, operation="mode", mode="stepped")
...
| `quake_status` | Connection, instance and bridge state |
...
| `quake_game` | `new_game`, `restart`, `load_map`, `save`, `load`, `list_maps`, `list_saves` |
...
- The bridge serializes mutations through a controller lease (2 s expiry,
  ...
  `action_id`, and a monotonic `action_seq`; retrying an `action_id` returns
  the recorded result instead of executing twice.
...
never logged or passed on a command line. One client at a time. Console
...
- `kill` currently has no observable effect through the console allowlist, so
  death, respawn and intermission reachability are unverified
  (`quake_game respawn` stays unsupported).
