/*
gl_vidsdl.c — SDL3 video/GL driver for GLQuake on macOS arm64.
Replaces gl_vidlinuxglx.c (X11/GLX/DGA). Structure mirrors that file
section by section; only the windowing calls change. Input (IN_* and
the SDL event pump) lives in in_sdl.c; sdl_window is non-static
because that module needs it.
...
/* menu.c's M_Draw locks/unlocks around console background drawing;
   gl_vidlinuxglx.c provides these as empty stubs for GL, and this
